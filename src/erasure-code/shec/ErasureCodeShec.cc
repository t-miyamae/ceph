				   ...
				   map<int, bufferlist> *decoded)
{
  ...
  if (want_count > 0) {
    return shec_decode(want, avails, data, coding, blocksize);
  } else {
    return 0;
  }
}
...
int ErasureCodeShecReedSolomonVandermonde::shec_decode(int *want,
                                            int *avails,
                                            char **data,
                                            char **coding,
                                            int blocksize)
{
  return shec_matrix_decode(want, avails, data, coding, blocksize);
}
...
int ErasureCodeShec::shec_make_decoding_matrix(bool prepare, int *want, int *avails,
					       ...
					       int *minimum)
{
  ...
  for (unsigned long long pp = 0; pp < (1ull << m); ++pp) {
    ...
    // Can selected parity chunks recover erased data chunks.
    bool ok = true;
    ...
    // Data chunks which are used to recovery erased parity chunks, are added to tmprow.
    for (int i = 0; i < m; i++) {
      if (want[k + i] == 1) {
        for (int j = 0; j < k; j++) {
          if (matrix[i * k + j] != 0 && avails[j] != 0) {
	    tmprow[j] = 1;
	    tmpcolumn[j] = 1;
	  }
        }
